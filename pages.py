        ...
        try:
            ...
            if not df.empty:
                ...
                if "numModels" in df.columns:
                    df["datasetCompletenessRatio"] = _safe_completeness_ratio(df.get("numModelsWithDataset", 0), df["numModels"])
                    df["collectionCompletenessRatio"] = _safe_completeness_ratio(df.get("numModelsWithCollection", 0), df["numModels"])
                    df["spaceCompletenessRatio"] = _safe_completeness_ratio(df.get("numModelsWithSpace", 0), df["numModels"])
                    df["benchmarkCompletenessRatio"] = _safe_completeness_ratio(df.get("numModelsWithBenchmark", 0), df["numModels"])
                else:
                ...
                total_models = float(pd.to_numeric(df.get("numModels", 0), errors="coerce").fillna(0).sum())
                dataset_linked_models = float(pd.to_numeric(df.get("numModelsWithDataset", 0), errors="coerce").fillna(0).sum())
                collection_linked_models = float(pd.to_numeric(df.get("numModelsWithCollection", 0), errors="coerce").fillna(0).sum())
                space_linked_models = float(pd.to_numeric(df.get("numModelsWithSpace", 0), errors="coerce").fillna(0).sum())
                benchmark_linked_models = float(pd.to_numeric(df.get("numModelsWithBenchmark", 0), errors="coerce").fillna(0).sum())
                ...
                agg_ratio_df = pd.DataFrame(
                    {
                        "Dimension": ["Dataset", "Collection", "Space", "Benchmark"],
                        "Linked models": [
